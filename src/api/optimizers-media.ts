...
export async function getMediaSettings(): Promise<MediaFormData> {
  const response = await fetch(`${wpApiSettings.root}cache-hive/v1/optimizers/media`, {
    method: 'GET',
    headers: {
...
export async function updateMediaSettings(data: MediaFormData): Promise<MediaFormData> {
  const response = await fetch(`${wpApiSettings.root}cache-hive/v1/optimizers/media`, {
    method: 'POST',
    headers: {
      'X-WP-Nonce': wpApiSettings.nonce,
