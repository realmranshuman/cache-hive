...
export async function getHtmlSettings(): Promise<HtmlFormData> {
  const response = await fetch(`${wpApiSettings.root}cache-hive/v1/optimizers/html`, {
    method: 'GET',
    headers: {
...
export async function updateHtmlSettings(data: HtmlFormData): Promise<HtmlFormData> {
  const response = await fetch(`${wpApiSettings.root}cache-hive/v1/optimizers/html`, {
    method: 'POST',
    headers: {
      'X-WP-Nonce': wpApiSettings.nonce,
