...
export async function getCssSettings(): Promise<CssFormData> {
  const response = await fetch(`${wpApiSettings.root}cache-hive/v1/optimizers/css`, {
    method: 'GET',
    headers: {
...
export async function updateCssSettings(data: CssFormData): Promise<CssFormData> {
  const response = await fetch(`${wpApiSettings.root}cache-hive/v1/optimizers/css`, {
    method: 'POST',
    headers: {
      'X-WP-Nonce': wpApiSettings.nonce,
